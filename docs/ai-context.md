...
- **Target URLs use Docker DNS names**, not `localhost`. All scripts must use `http://prism:8080/...` — never `http://localhost:8080/...`.
...
| Prism as SUT | OpenAPI mock server — validates requests against spec; supports `/delay/N` for latency simulation |
...
4. Use `http://prism:8080/<endpoint>` as target.
...
- Target: `http://prism:8080/` + valid Prism endpoint (must exist in `openapi.yml`).
