...
- **Prism** (SUT, `stoplight/prism:5`) — the HTTP mock target for all tests
...
- **Target URLs must use Docker service names**, not `localhost`. Always `http://prism:8080/...`.
...
// NEVER — no check
export default function() { http.get('http://prism:8080/get'); }
```
